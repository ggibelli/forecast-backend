...
surfRouter.get('/', async (req, res) => {
  const surfSpots = await Continent.find({}).populate({
    path: 'countries',
...
surfRouter.get('/surfspots', async (req, res) => {
  const spots = await SurfSpot.find({ isSecret: false })
    .select('name')
...
surfRouter.get('/continents/:id', async (req, res) => {
  const continents = await Continent.findById(req.params.id).populate({
    path: 'countries',
    select: 'name',
    populate: {
      path: 'regions',
...
surfRouter.get('/regions/:id', async (req, res) => {
  const regions = await Region.findById(req.params.id)
    .populate(
      'surfSpots',
...
surfRouter.get('/surfspots/:id', async (req, res) => {
  const spot = await SurfSpot.findOne({ _id: req.params.id })
    .populate('continent', { name: 1 })
    .populate('country', { name: 1 })
    .populate('region', { name: 1 })
    .populate('forecast', { id: 1 })
  if (!spot) throw new SurfSpotNotFoundError()
  spot.latitude !== 'unknown' &&
    !spot.forecast &&
    (await forecastHelper.createForecast(spot))
  res.json(spot)
})
...
surfRouter.put('/surfspots/:id', async (req, res) => {
  ...
  if (!longitudeIsValid(req.body.longitude)) throw new InvalidLongitude()
  const user = await User.findById(decodedToken.id)
  if (spot.user !== user.id.toString())
    throw new AuthenticationError('Only surfspot creator can modify it')
  const updatedSpot = await SurfSpot.findByIdAndUpdate(req.params.id, spot, {
    new: true,
  })
  await updatedSpot
    .populate('continent', { name: 1 })
...
surfRouter.delete('/surfspots/delete/:id', async (req, res) => {
  ...
  if (!req.token || !decodedToken.id) throw new InvalidToken()
  const user = await User.findById(decodedToken.id)
  if (spot.user.toString() !== user.id.toString())
    throw new AuthenticationError('Only surfspot creator can delete it')
  await spot.remove()
  user.createdSpots = user.createdSpots.filter(
    (spot) => spot.id.toString() !== req.params.id.toString()
  )
  await user.save()
  res.status(204).end()
})
